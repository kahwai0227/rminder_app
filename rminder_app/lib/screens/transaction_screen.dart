...
class _TransactionsPageState extends State<TransactionsPage> {
  ...
  List<models.Transaction> _getFilteredTransactions(AppState state) {
    final debtCategoryIds = state.liabilities
        .map((l) => l.budgetCategoryId)
        .toSet();
    return state.transactions.where((txn) {
      final byCategory =
          _filterCategory == null || txn.categoryId == _filterCategory!.id;
      final byDebt = !_hideDebt || !debtCategoryIds.contains(txn.categoryId);
      bool byDate = true;
      if (_filterSingleDate != null) {
        byDate =
            txn.date.year == _filterSingleDate!.year &&
            txn.date.month == _filterSingleDate!.month &&
            txn.date.day == _filterSingleDate!.day;
      } else if (_filterStartDate != null && _filterEndDate != null) {
        byDate =
            !txn.date.isBefore(_filterStartDate!) &&
            !txn.date.isAfter(_filterEndDate!);
      } else if (_filterMonthDate != null) {
        byDate =
            txn.date.year == _filterMonthDate!.year &&
            txn.date.month == _filterMonthDate!.month;
      }
      bool byAmount = true;
      if (_filterMinAmount != null) byAmount = txn.amount >= _filterMinAmount!;
      if (_filterMaxAmount != null)
        byAmount = byAmount && txn.amount <= _filterMaxAmount!;
  ...
  (String display, bool isPositive) _signedAmountForDisplay(
    AppState state,
  ...
  List<Widget> _buildGroupedTransactionWidgets(
    ...
    for (final t in txns) {
      ...
      final isDebt = state.liabilities.any(
        (l) => l.budgetCategoryId == t.categoryId,
      );
  ...
  Future<void> _showAddTransactionDialog(AppState state) async {
    models.BudgetCategory? selectedCategory = state.categories.isNotEmpty
        ? state.categories[0]
        : null;
  ...
  Future<void> _showEditTransactionDialog(
    ...
    models.BudgetCategory? selectedCategory = state.categories.firstWhere(
  ...
  Future<void> _showFilterDialog(AppState state) async {
    final result = await showDialog<Map<String, dynamic>>(
      ...
      builder: (context) {
        ...
        double currentMin = (tempMinAmount ?? minAvail).clamp(
          minAvail,
          maxAvail,
        );
        double currentMax = (tempMaxAmount ?? maxAvail).clamp(
          currentMin,
          maxAvail,
        );
                ...
                child: Column(
                  ...
                  children: [
                    DropdownButton<models.BudgetCategory>(
                      ...
                      onChanged: (cat) =>
                          setDialogState(() => tempCategory = cat),
                    ),
                    const SizedBox(height: 8),
                    FilterChip(
                        ...
                        ChoiceChip(
                          ...
                          onSelected: (_) {
                            setDialogState(() {
                              ...
                              tempEndDate = null;
                              tempMonthDate =
                                  tempMonthDate ?? monthOptions.first;
                            });
                          },
                        ),
                      ],
                    ...
                    if (dateMode == 'single')
                      InkWell(
                        onTap: () async {
                          ...
                          if (picked != null)
                            setDialogState(() => tempSingleDate = picked);
                        },
                        child: Container(
                          padding: const EdgeInsets.symmetric(
                            horizontal: 12.0,
                    ...
                    if (dateMode == 'month')
                      InkWell(
                        onTap: () async {
                                ...
                                builder: (ctx, setInner) {
                                  return DropdownButton<DateTime>(
                                    ...
                                    items: monthOptions
                                        ...
                                        .toList(),
                                    onChanged: (dt) => setInner(
                                      () => localSel = dt ?? localSel,
                                    ),
                                  );
                                },
                              ),
                          ...
                          if (selected != null)
                            setDialogState(() => tempMonthDate = selected);
                        },
                        child: Container(
                          padding: const EdgeInsets.symmetric(
                            horizontal: 12.0,
                    ...
                    RangeSlider(
                      ...
                      onChanged: (values) {
                        setDialogState(() {
                          ...
                          minAmountController.text = values.start
                              .toStringAsFixed(0);
                          maxAmountController.text = values.end.toStringAsFixed(
                            0,
                          );
                        });
                      },
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      ElevatedButton.icon(
                        ...
                        onPressed: appState.categories.isEmpty
                            ? null
                            : () => _showAddTransactionDialog(appState),
                      ),
                    ],
                  ),
                  const SizedBox(height: 10),
